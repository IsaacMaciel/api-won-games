...
async function getGameInfo(slug) {
  try {
    ...
    console.info(`getGameInfo: ${error}`);
  }
}
...
async function createManyToManyData(products) {
  try {
    ...
    console.info(`createManyToManyData: ${error}`);
  }
}
  ...
  try {
    await Promise.all(
      products.map(async (product) => {
        ...
        if (!item) {
          console.info(`Criando: ${product.title}...`);
    ...
    console.info(`createGames: ${error}`);
  }
}
...
module.exports = {
  populate: async (params) => {
    try {
      const GOG_URL = `https://www.gog.com/games/ajax/filtered?mediaType=game&${qs.stringify(
        params
      )}`;
      const {
        data: { products },
      } = await axios.get(GOG_URL);
